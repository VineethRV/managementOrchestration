...
def sanitize_filename(name: str) -> str:
    ...
    IMPORTANT: NO hyphens allowed in file names. Uses PascalCase (e.g., UserProfile).
    """
    import re
    # Remove special characters, replace hyphens and spaces with underscores first
    ...
    # Return PascalCase (no underscores or hyphens in final result)
...
def sanitize_backend_filename(name: str) -> str:
    ...
    MANDATORY: Uses ONLY underscores (_), NEVER hyphens (-) in file names.
    """
    import re
    # Convert to lowercase first
    name = name.lower()
    # Replace ALL hyphens and spaces with underscores
    name = name.replace('-', '_').replace(' ', '_')
...
def generate_projects(design: Dict[str, Any], project_name: str = "project"):
    """
    Generate both React frontend and Flask backend projects.
    
    Args:
        design: Complete design dictionary with frontend/backend
        project_name: Name of the root project folder (default: "project")
    ...
    # Define paths for frontend and backend within project folder
    frontend_path = os.path.join(project_name, "frontend")
    backend_path = os.path.join(project_name, "backend")
    
    # Generate React frontend in project/frontend
    frontend_success = create_react_project(design, frontend_path)
    
    # Generate Flask backend in project/backend
    backend_success = create_flask_backend(design, backend_path)
    ...
    if frontend_success and backend_success:
        ...
        print(f"   ├── frontend/   (React app)")
        print(f"   └── backend/    (Flask API)")
        print("\nNext steps:")
        print("\n1. Start the backend:")
        print(f"   cd {project_name}/backend")
    ...
    return {
        "react": os.path.abspath(frontend_path) if frontend_success else None,
        "flask": os.path.abspath(backend_path) if backend_success else None
    }
